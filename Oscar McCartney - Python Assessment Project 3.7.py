...
class GrapherGUI:
    ...
    def __init__(self, parent):
        """Define all the variables and widgets that make up the grapher."""
        np.seterr(divide = "ignore")  # stop divide by 0 and log -ve number errors
        ...
        self.eq_var.trace("w", self.draw)  # call self.draw every time self.entry is changed by the user
    ...
    def draw(self, *args):
        """Validate the equation input, save the equation to the equation.txt file and draw the graph.
        ...
        if len(args) == 1:
            ...
            self.num_period = [x[1] for x in self.zoom_points if x[0] <= self.scale][0]  # change the spacing of the numbers on the x and y axes            
        ...
        try:
            error_char = eq_list[list(np.compare_chararrays(eq_list, (eq_valid_list+["" for _ in range(abs(len(eq_valid_list)-len(eq_list)))]), "==", False)).index(False)]  # find the first character in eq_list that isn't valid
        except:
            error_char = ""
        ...
        if error_char == "":
            ...
            try:
                for i in range(0, 512):
                    ...
                    x = ((i)-256)/self.scale  # Shift the y-axis to the middle of the screen and scales the screen.
                ...
                if "log(" in self.eq:  # make sure that the asymptote for log graphs is drawn because my grapher doesn't calculate every y value, only 512.
                    self.points[0][1] = 512
    ...
    def canvas_reset(self):
        """Clear the canvas and draw the axis lines."""
        self.num_period_pixels = self.num_period*self.scale
        ...
        for x in np.arange(start, 512-start, self.num_period_pixels):
            ...
            if str(x_disp).split(".")[1] == "0": x_disp = int(x_disp)  # turn x_disp into an int if it ends in .0
            if x_disp != 0:
        ...
        for y in np.arange(start, 512-start, self.num_period_pixels):
            ...
            if str(y_disp).split(".")[1] == "0": y_disp = int(y_disp)  # turn y_disp into an int if it ends in .0
            if y_disp != 0:
    ...
    def zoom_in(self):
        """Zoom in by 0.1 everytime the + button is pressed."""
        self.zoom.set(self.zoom.get()+0.1)

    def zoom_out(self):
        """Zoom out by 0.1 everytime the - button is pressed."""
        self.zoom.set(self.zoom.get()-0.1)

    def mouse_zoom(self, event):
        """Zoom in or out by event.delta/2500 every time the scroll wheel is moved."""
